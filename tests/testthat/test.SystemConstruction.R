...
test_that("certain run_biocro inputs must have the correct type", {
    expect_error(
        run_biocro(
            unlist(soybean_initial_values),
            unlist(soybean_parameters),
            unlist(get_growing_season_climate(soybean_weather2005)),
            soybean_direct_modules,
            soybean_differential_modules,
            unlist(soybean_ode_solver['type'])
        ),
        regexp = paste0(
...
test_that("certain run_biocro inputs must not have empty elements", {
    expect_error(
        run_biocro(
            within(soybean_initial_values, {bad_initial_value = numeric(0)}),
            within(soybean_parameters, {bad_parameter = numeric(0)}),
            get_growing_season_climate(soybean_weather2005),
            within(soybean_direct_modules, {bad_direct_module = character(0)}),
            within(soybean_differential_modules, {bad_differential_module = character(0)}),
            within(soybean_ode_solver, {bad_ode_solver_setting = numeric(0)})
        ),
        regexp = paste0(
...
test_that("certain run_biocro inputs must not have elements with length > 1", {
    expect_error(
        run_biocro(
            within(soybean_initial_values, {bad_initial_value = c(1,2)}),
            within(soybean_parameters, {bad_parameter = c(1,2)}),
            get_growing_season_climate(soybean_weather2005),
            soybean_direct_modules,
            soybean_differential_modules,
            within(soybean_ode_solver, {bad_ode_solver_setting = c(1,2)})
        ),
        regexp = paste0(
            "The following `initial_values` members have lengths other than 1, but all parameters must have a length of exactly 1: bad_initial_value.\n",
            "  The following `parameters` members have lengths other than 1, but all parameters must have a length of exactly 1: bad_parameter.\n",
...
test_that("certain run_biocro inputs must be strings", {
    expect_error(
        run_biocro(
            ...
            soybean_parameters,
            get_growing_season_climate(soybean_weather2005),
            append(soybean_direct_modules, 1.23),
            append(soybean_differential_modules, 4.56),
            within(soybean_ode_solver, {type = 7.89})
        ),
        regexp = paste0(
...
test_that("certain run_biocro inputs must be numeric", {
    expect_error(
        run_biocro(
            within(soybean_initial_values, {bad_initial_value = "terrible"}),
            within(soybean_parameters, {bad_parameter = "awful"}),
            within(get_growing_season_climate(soybean_weather2005), {bad_driver = "offensive"}),
            soybean_direct_modules,
            soybean_differential_modules,
            within(soybean_ode_solver, {bad_ode_solver_setting = "heinous"})
        ),
        regexp = paste0(
